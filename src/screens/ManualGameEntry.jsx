      ...
      <div className="min-h-dvh pb-8">
        ...
        <div className="px-4 py-4 space-y-4">
          ...
          <div>
            ...
            {(() => {
              const totalGoals = points.reduce((sum, pt) => sum + (pt.stats || []).filter(s => s.type === 'goal' || s.type === 'score').length, 0);
              const totalAssists = points.reduce((sum, pt) => sum + (pt.stats || []).filter(s => s.type === 'assist').length, 0);
              const totalDs = points.reduce((sum, pt) => sum + (pt.stats || []).filter(s => s.type === 'D' || s.type === 'd').length, 0);
              const net = ourScore - theirScore;
              return (
                <div className="flex items-center justify-between bg-navy-800 rounded-lg px-3 py-2 mb-4">
                  <div className="flex gap-4 text-xs">
                    <div className="text-center"><div className="text-navy-400 mb-0.5">PF</div><div className="font-bold text-score-green">{ourScore}</div></div>
                    <div className="text-center"><div className="text-navy-400 mb-0.5">PA</div><div className="font-bold text-score-red">{theirScore}</div></div>
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {[...checkedInPlayerIds].map(pid => {
                ...
                return (
                  <div key={pid} className="flex items-center text-sm bg-navy-800 rounded-lg px-3 py-2">
                    <div className="flex items-center gap-2 min-w-0 flex-1">
